...
from cotkg_ids.knowledge_graph.cot_generator import LLMProvider, generate_cot, parse_cot_response
...
class TestLLMIntegration(unittest.TestCase):
    ...
    def test_provider_initialization(self):
        ...
        for model_name, config in provider.models.items():
            api_key = os.getenv(config.api_key_env) if hasattr(config, 'api_key_env') else None
            if api_key or model_name == 'ollama':  # Ollama doesn't need an API key
    ...
    def test_zhipu_integration(self):
        ...
        if "zhipu" not in provider.clients:
            ...
            self.skipTest("Skipping Zhipu test - API integration needs to be updated")

    def test_generate_cot_fallback(self):
        """Test the fallback mechanism of generate_cot"""
        try:
            response = generate_cot(self.sample_flow)
            self.assertIsInstance(response, str)
            self.logger.info("CoT generation successful")
        except Exception as e:
    ...
    def test_parse_cot_response(self):
        ...
        1. Attack Type: DDoS Attack
           Severity: High
        2. Feature: High packet count
           Value: Excessive
        3. Feature: Unusual port
           Value: Anomalous

        Relationships:
        1. High packet count INDICATES DDoS Attack
        2. Unusual port INDICATES DDoS Attack
        """
