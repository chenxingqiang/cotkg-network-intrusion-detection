...
# Windows (需要以管理员权限运行 PowerShell)
.\setup_neo4j.sh
```

注意事项：

1. 对于 Windows，需要先安装 Chocolatey 包管理器
2. 脚本需要管理员/root 权限运行
3. 默认密码设置为 "neo4jneo4j"
4. 脚本会自动安装所需的依赖（Java 17）
5. 安装完成后会自动验证安装并显示访问信息

验证成功后，你可以：

1. 通过 <http://localhost:7474> 访问 Neo4j 浏览器
2. 使用以下凭据登录：
   - 用户名：neo4j
   - 密码：neo4jneo4j

如果遇到问题，脚本会指示你查看相应的日志文件位置。
